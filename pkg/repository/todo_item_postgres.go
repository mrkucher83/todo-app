...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	tx, err := r.db.Begin() // метод Begin() окрывает транзакцию для выподнения нескольких запросов
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) Delete(userId, itemId int) error {
	query := fmt.Sprintf(
		"DELETE FROM %s ti USING %s li, %s ul WHERE ti.id=li.item_id AND ul.user_id=$1 AND ti.id=$2",
		todoItemsTable, listsItemsTable, usersListsTable)
